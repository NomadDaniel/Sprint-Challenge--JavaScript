...
const universities = (graduates) => 
  graduates.map(ele => ele.university)
    console.log(universities(graduates));

    // const universities = [];
    //   for (let university = 0; university < graduates.length; university++) {
    //     universities.push(graduates[university].university);
    //   }
    // universities.sort();
...
// used .search, for request 3, also played around w .match but couldn't get to work
...
const displayNames = [];
  zooAnimals.forEach(ele => {
    displayNames.push(`Name: ${ele.animal_name}, Scientific: ${ele.scientific_name}`);
  }
);
...
*/
const populationTotal = zooAnimals.reduce((initialVal, count) => {
  return initialVal + count.population}, 0);
